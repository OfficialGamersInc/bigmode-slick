...
signal died(health_delta : float)
signal health_changed(health_delta : float)
signal knockback_requested(force : Vector3)

func change_health(health_delta : float, source_dir : Vector3 = Vector3.ZERO, knockback : float = 0) -> void:
	#health += health_delta
	...
	if knockback > 0:
		knockback_requested.emit(-source_dir.normalized() * knockback)
