...
@onready var view_occlusion_cast : ShapeCast3D = find_child("ViewOcclusionCast", false, true) #$ViewOcclusionCast
...
func _process(delta):
	...
	if view_occlusion_cast and view_occlusion_cast.is_colliding():
...
func refresh_view_occlusion_cast(offset : Vector3):
	if not view_occlusion_cast: return null
